module capu_dma_negative_completion_v28 #(
    ...
    output logic negative_receipt_valid,
    output logic [CMD_WIDTH-1:0] negative_receipt_command_id,
    output logic [EXEC_EPOCH_WIDTH-1:0] negative_receipt_execution_epoch,
    output logic [EFFECT_WIDTH-1:0] negative_receipt_effect_id,
    ...
    logic matching_negative_receipt_live, matching_negative_receipt_checkpoint;
    logic matching_completion_receipt_live, matching_completion_receipt_checkpoint;
    logic negative_receipt_conflict;
    ...
    assign matching_negative_receipt_live = negative_receipt_valid && command_pending &&
    ...
    assign matching_negative_receipt_checkpoint = negative_receipt_valid && checkpoint_valid && checkpoint_command_pending &&
    ...
    assign negative_receipt_conflict = negative_receipt_valid && command_pending && !matching_negative_receipt_live;

    assign submit_accept = submit_valid && runtime_ready && !command_pending &&
        !checkpoint_valid && !issue_receipt_valid && !negative_receipt_valid && !completion_receipt_valid &&
    ...
    assign dma_replay_authority = runtime_ready && command_pending &&
        ...
        !issue_receipt_valid && !completion_receipt_valid && !negative_receipt_conflict &&
        !recovery_begin && !restore_valid;
    ...
    always_ff @(posedge clk or negedge rst_n) begin
        if(!rst_n) begin
            ...
            negative_receipt_valid <= 1'b0;
            negative_receipt_command_id <= '0;
            negative_receipt_execution_epoch <= '0;
            negative_receipt_effect_id <= '0;
            ...
            if(matching_completion_receipt_checkpoint) begin
                ...
                issue_receipt_valid <= 1'b0;
                negative_receipt_valid <= 1'b0;
            end else if(matching_negative_receipt_checkpoint) begin
                dma_issued <= 1'b0;
                completion_state <= NOT_COMMITTED;
                evidence_required <= 1'b0;
                issue_receipt_valid <= 1'b0;
