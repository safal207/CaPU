module capu_queue_epoch_wrap_incarnation_v33_live_formal;
  logic clk;
  ...
  always @(posedge clk) begin
    ...
    if(rst_n) begin
      ...
      if(stale_evidence_quarantine_accept&&runtime_ready&&!recovery_begin&&!restore_valid)
    ...
    if(past_valid&&$past(rst_n)&&$past(stale_evidence_quarantine_accept&&runtime_ready&&!recovery_begin&&!restore_valid))
      assert(stale_evidence_quarantined);

    cover(rst_n&&epoch_wrap_accept);
    cover(rst_n&&stale_evidence_quarantine_accept&&runtime_ready&&!recovery_begin&&!restore_valid&&resolution_queue_epoch==durable_queue_epoch&&resolution_incarnation<durable_incarnation);
