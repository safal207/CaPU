module capu_dma_negative_completion_v28_formal;
  logic clk;
  ...
  (* anyseq *) logic dma_issue_valid; (* anyseq *) logic [1:0] dma_issue_command_id,dma_issue_execution_epoch,dma_issue_effect_id;
  (* anyseq *) logic resolution_valid,resolution_committed; (* anyseq *) logic [1:0] resolution_command_id,resolution_execution_epoch,resolution_effect_id;
  ...
  logic issue_receipt_valid; logic [1:0] issue_receipt_command_id,issue_receipt_execution_epoch,issue_receipt_effect_id;
  logic negative_receipt_valid; logic [1:0] negative_receipt_command_id,negative_receipt_execution_epoch,negative_receipt_effect_id;
  logic completion_receipt_valid; logic [1:0] completion_receipt_command_id,completion_receipt_execution_epoch,completion_receipt_effect_id;
  ...
  wire matching_negative_live = negative_receipt_valid && command_pending &&
    negative_receipt_command_id==live_command_id &&
    negative_receipt_execution_epoch==live_execution_epoch &&
    negative_receipt_effect_id==live_effect_id;
  wire matching_negative_checkpoint = negative_receipt_valid && checkpoint_valid && checkpoint_command_pending &&
    negative_receipt_command_id==checkpoint_command_id &&
    negative_receipt_execution_epoch==checkpoint_execution_epoch &&
    negative_receipt_effect_id==checkpoint_effect_id;
  wire matching_completion_checkpoint = completion_receipt_valid && checkpoint_valid && checkpoint_command_pending &&
  ...
  capu_dma_negative_completion_v28 #(
    .CMD_WIDTH(2),
    .EXEC_EPOCH_WIDTH(2),
    .EFFECT_WIDTH(2)
  ) dut(.*);
  ...
  always @(posedge clk) begin
    ...
    if(rst_n) begin
      ...
      if(dma_replay_authority) begin
        ...
        assert(!issue_receipt_valid && !completion_receipt_valid);
        if(negative_receipt_valid) assert(matching_negative_live);
      end
      if(negative_receipt_valid && command_pending) begin
        assert(matching_negative_live);
        assert(completion_state==2'b00);
        assert(!dma_issued && !evidence_required);
      ...
      if(dma_issue_accept) begin
        assert(command_pending && completion_state==2'b00);
    ...
    if(past_valid && $past(rst_n) && $past(resolution_accept)) begin
      ...
      if($past(resolution_committed)) begin
        ...
        assert(completion_state==2'b00);
        assert(negative_receipt_valid);
        assert(negative_receipt_command_id==$past(live_command_id));
        assert(negative_receipt_execution_epoch==$past(live_execution_epoch));
        assert(negative_receipt_effect_id==$past(live_effect_id));
        if(!recovery_begin && !restore_valid)
          assert(dma_replay_authority || !command_pending);
      end
    end
    ...
    if(past_valid && $past(rst_n) && $past(recovery_begin)) begin
      ...
      assert(negative_receipt_valid==$past(negative_receipt_valid));
      assert(completion_receipt_valid==$past(completion_receipt_valid));
      if($past(negative_receipt_valid)) begin
        assert(negative_receipt_command_id==$past(negative_receipt_command_id));
        assert(negative_receipt_execution_epoch==$past(negative_receipt_execution_epoch));
        assert(negative_receipt_effect_id==$past(negative_receipt_effect_id));
      end
    end

    if(past_valid && $past(rst_n) && $past(restore_accept) &&
       $past(matching_negative_checkpoint) && !$past(matching_completion_checkpoint)) begin
      assert(runtime_ready && command_pending);
      assert(completion_state==2'b00);
      assert(!evidence_required && !dma_issued);
      assert(negative_receipt_valid);
      assert(!issue_receipt_valid);
      if(!recovery_begin && !restore_valid)
        assert(dma_replay_authority);
    ...
    if(past_valid && $past(rst_n) && $past(restore_accept) && $past(matching_completion_checkpoint)) begin
      assert(runtime_ready && command_pending);
      assert(completion_state==2'b10);
      assert(!evidence_required);
      assert(!negative_receipt_valid);
    ...
    cover(rst_n && recovery_begin && negative_receipt_valid);
    cover(rst_n && restore_accept && matching_negative_checkpoint);
    cover(rst_n && dma_issue_accept && matching_negative_live);
    cover(rst_n && resolution_accept && resolution_committed);
