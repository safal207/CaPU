module capu_durable_negative_completion_v28 #(
    ...
    assign dma_replay_authority = runtime_ready && command_pending &&
        ...
        (!negative_receipt_valid || matching_negative_receipt_live) &&
        !recovery_begin && !restore_valid;
    ...
    always_ff @(posedge clk or negedge rst_n) begin
        if(!rst_n) begin
            ...
            if(matching_completion_receipt_checkpoint) begin
                ...
                evidence_required <= 1'b0;
            end else if(matching_issue_receipt_checkpoint) begin
                dma_issued <= 1'b1;
                ...
                completion_state <= NOT_COMMITTED;
                evidence_required <= 1'b0;
            end else if(checkpoint_completion_state == UNKNOWN) begin
                dma_issued <= 1'b1;
                completion_state <= UNKNOWN;
                evidence_required <= 1'b1;
            end else begin
