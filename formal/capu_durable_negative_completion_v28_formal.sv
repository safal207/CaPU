module capu_durable_negative_completion_v28_formal;
  logic clk;
  ...
  capu_durable_negative_completion_v28 #(
    .CMD_WIDTH(2),
    .EXEC_EPOCH_WIDTH(2),
    .EFFECT_WIDTH(2)
  ) dut(.*);
  ...
  always @(posedge clk) begin
    ...
    if(rst_n) begin
      ...
      assert(!(issue_receipt_valid && negative_receipt_valid));
      assert(!(negative_receipt_valid && completion_receipt_valid));
      ...
      if(dma_replay_authority) begin
        ...
        if(negative_receipt_valid) begin
          assert(negative_receipt_command_id==live_command_id);
          assert(negative_receipt_execution_epoch==live_execution_epoch);
          assert(negative_receipt_effect_id==live_effect_id);
        end
      end
      if(negative_receipt_valid && command_pending &&
         negative_receipt_command_id==live_command_id &&
         negative_receipt_execution_epoch==live_execution_epoch &&
         negative_receipt_effect_id==live_effect_id) begin
        assert(completion_state==2'b00);
        assert(!evidence_required);
        assert(!dma_issued);
      end
    ...
    if(past_valid && $past(rst_n) && $past(restore_accept) &&
       $past(negative_receipt_valid && checkpoint_valid && checkpoint_command_pending &&
    ...
    cover(rst_n && checkpoint_capture_accept && completion_state==2'b01);
    cover(rst_n && resolution_accept && !resolution_committed);
    cover(rst_n && recovery_begin && negative_receipt_valid);
    cover(rst_n && restore_accept && negative_receipt_valid && checkpoint_completion_state==2'b01);
    cover(rst_n && dma_issue_accept && negative_receipt_valid);
    cover(rst_n && resolution_accept && resolution_committed);
