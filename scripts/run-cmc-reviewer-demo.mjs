...
const steps = [
  ...
  {
    name: 'runtime HTTP API manifest validation',
    command: 'npm',
    args: ['run', 'validate:runtime-http-manifest'],
    cwd: repoRoot,
    proves: 'The runtime HTTP API manifest maps routes, cases, boundaries, and fixtures to existing checked evidence.',
  },
  {
  ...
  {
    name: 'capu p6 fixture verifier',
    command: 'cargo',
    args: ['run', '--bin', 'capu_p6_fixture_verify', '--locked'],
    cwd: cmcDir,
    proves: 'Saved sealed CaPU P6 audit fixtures can be replay-verified and tamper-detected.',
  },
  {
    name: 'capu p6 action variants verifier',
    command: 'cargo',
    args: ['run', '--bin', 'capu_p6_action_variants_verify', '--locked'],
    cwd: cmcDir,
  ...
  {
    name: 'capu p1 persona memory verifier',
    command: 'cargo',
    args: ['run', '--bin', 'capu_p1_persona_memory_verify', '--locked'],
    cwd: cmcDir,
  ...
  {
    name: 'capu runtime sidecar smoke',
    command: 'cargo',
    args: ['run', '--bin', 'capu_runtime_sidecar_smoke', '--locked'],
    cwd: cmcDir,
