...
fn main() -> ExitCode {
    ...
    let sealed = seal_trace(&trace_jsonl);

    println!("CMC-VERIFY-TRACE hash-chain verifier v0");
    println!("hash=fnv1a64-dev-only");
    println!("events={}", sealed.len());

    for (idx, (prev_hash, event, trace_hash)) in sealed.iter().enumerate() {
        println!("event={} prev_hash={} trace_hash={} canonical={}", idx + 1, prev_hash, trace_hash, event);
    }

    match verify_trace(&sealed) {
        Ok(()) => {
            println!("result=trace_valid");
            println!("note=developer integrity demo only; not cryptographic evidence");
            ExitCode::SUCCESS
        }
        Err(seq) => {
            eprintln!("result=tampering_detected seq={seq}");
            ExitCode::FAILURE
        }
    }
}
